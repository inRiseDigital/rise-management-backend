...
from .serializers import LocationSerializer, TaskSerializer, SubcategoriesSerializer
...
class TaskByLocationView(APIView):
    ...
    def get(self, request, location_id):
        ...
        tasks = location.task.all()  # related_name='task'
...
class TaskReportByPeriodPDFView(APIView):
    ...
    def get(self, request):
        ...
        for location in grouped.values():
            ...
            data = [['No', 'Subcategory', 'Cleaning Type', 'Description', 'Update Date']]
            for idx, task in enumerate(location['tasks'], start=1):
                data.append([
                    idx,
                    task.subcategory,
                    task.cleaning_type,
                    task.description[:40],  # limit long text
